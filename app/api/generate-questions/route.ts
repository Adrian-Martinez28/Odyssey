...
const budgetMaxValues = [2000000, 3000000, 4000000, 6000000, 10000000];

export async function GET(req: Request) {
  // Generate 5 questions in JSON.
  const { text } = await generateText({
    seed: Number.parseInt(crypto.randomUUID().replace(/-/g, ''), 16),
    model: google("gemini-2.0-flash-lite-preview-02-05"),
    maxRetries: 3,
    prompt: `Genera 10 preguntas técnicas y específicas para recomendar un portátil en formato JSON, tienen que ser preguntas totalmente cerradas, el usuario no va a responder nada manualmente, solo las opciones que le das, incluye siempre una opcion no tengo preferencia.
...
No te limites a estas preguntas, puedes agregar otras que consideres importantes, no preguntes por presupuesto, eso lo haremos después.
